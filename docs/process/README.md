...
| [`1_level-1-macro-processes.md`](./1_level-1-macro-processes.md) | The four macro processes, each with its children in sequence |
...
flowchart TD
  ...
  classDef business fill:#fffbb5,stroke:#c8c04a,color:#333
  class m1,m2,m3,m4 business
```

| ID | Band | Macro process | Purpose | Composed of |
| -- | ---- | ------------- | ------- | ----------- |
| `BPROC1` | Operational | Establish the architecture model | Turns a subject nobody has modeled into a populated, approved model the next change can be judged against | `BPROC1.1` · `BPROC1.2` · `BPROC1.3` · `BPROC1.4` |
...
One branch of ten is detailed to level 3. That ratio is the point of the
