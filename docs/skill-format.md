...
**A skill states the behaviour, not the argument for it.**
...
only thing loaded before a skill is chosen, so it is the only place a kind marker
can change what the agent reaches for. The title repeats it as a glyph — `# ⚙`,
`# ▤`, `# ※` — for whoever opens the file. `check_skills.py` checks both against
`kind`.
...
anyone who installed rather than cloned. A consuming project's document is named
in a code span instead — `` `architecture/README.md` ``.
...
A rulebook is loaded by every skill that obeys it, so its size is paid on almost
every activation; moving lookup tables into references is how that cost is kept
down without deleting a rule.
...
- **A citation names a heading, not a file.** `` `skill` § Heading `` resolves
  ...
  reference costs no edit anywhere else.
...
which models a skill as a schema-validated execution graph. What is taken from
it is **naming the negative space** — a skill states when *not* to use it, what
it hands off to and gets back, the mistakes it steers away from, and the rules
holding across every step — and that a skill belongs to a **kind** which decides
what structure it owes, which is why `REQUIRED_SECTIONS` is keyed by kind.

**The format is markdown rather than AIP's fenced YAML.** AIP's value comes from
steps backed by scripts and wired by typed edges; these skills have neither, and
markdown keeps one artifact for both readers and keeps the diagrams. What
replaced the JSON Schemas is the section table above, and `check_skills.py`
enforcing it.
