...
| **Data objects** | The data domains and their owners — `Customer data`, `Product data` — one map, few boxes | Each object's owner, classification and where it is mastered, per subdomain where the domain splits | A schema. Attributes belong to the systems that store them; the model carries what a decision needs |
