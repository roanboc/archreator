...
each element represents). The definitions are quoted from the ArchiMate 3.2
Specification (© The Open Group). An edge label or a relationship cell prefers
the role name for its direction — *serves*, *realized by* — and a domain word
is allowed where it says more; nothing machine-maps labels onto this
vocabulary.
...
block is a fact living inside a rendering, which `P1` does not allow.
...
table" is prose that mentions one. That is what separates a relationship column
...
the parse reads it. Two rules make that safe:
