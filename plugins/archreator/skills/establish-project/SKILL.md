...
| `mkdocs.yml` | The site name, the description, the repository URL — so every published page carries a link back to the file that produced it — and `theme.language` when the project documents in something other than English. Left as it ships, the portal still builds — without those links. On a **public** repository the `extra.giscus` values belong here too, which is what turns the comment box on; anywhere else leave them unset, and the "Discuss this page" link in `overrides/main.html` is what a reader gets instead |
