...
- **Evidence or Pending — never plausibility.** Every element names where it
  ...
  answer. An application nobody could name the owner of is recorded as
  **"Pending — owner unknown"** rather than assigned to the team that probably
...
- **Describe what runs, not what should run.** The estate includes things
  ...
  `plan-the-transition`.
- **Coverage is declared, never implied.** A sweep is never finished; it is
  stopped somewhere on purpose. What was left out is written down beside what
  was covered, so a reader can tell a boundary from an omission.
...
and hand to `model-domains` — splitting a described model costs more than
splitting an empty one.

**→ Produces** the coverage declaration, as a draft table.

### 2 — Gather evidence, not opinions

Collect what already exists before asking anyone to remember anything.
...
`assets/layers/reference/` the first time — under its dated name, with its row
in that folder's index, *then* gets read.
...
definitions and the focus table.

**Actors include the AI ones.** A running organization has agents and
assistants embedded in processes without anyone modeling them as actors
holding roles. Record each one with its autonomy level and decision rights the
way any other actor is recorded.
...
the model rather than the scope document: a reader of `2_business/` will not
look in a merged scope document for what the sweep did not reach.
...
- **Writing down a reading of a person.** A transcript summary records
  ...
  whose team is difficult. See `architecture-document-style` § A summary of a
  meeting records facts, not judgements.
...
- Handing over a list of things three people mentioned as approved
  architecture.
- Reading a document somebody sent and not filing it.
