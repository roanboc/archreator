...
Two validators that keep this project's architecture documents honest, and one
projection for readers who cannot read Markdown. They came with the scaffold,
so this project has had them since its first commit, and CI should run both
validators on every pull request.
...
what failed. `build_model.py` is a tool rather than a gate: nothing has to be
green for it, and nothing breaks if it is never run.
...
| `model_graph.py` | Library, imported by the two above. The single parse of the document convention — element IDs, catalogue tables, Mermaid edges |
